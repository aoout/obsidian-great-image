...
export class ImageToolkitSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.realTimeUpdate).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Do not search or rename attachments in these folders. Write each folder on a new line."
			)
			.addTextArea((cb) =>
				cb
		...
		new Setting(containerEl)
			...
			.addText((text) =>
				text.setValue(this.plugin.settings.includedFileRegex).onChange(async (value) => {
					if (!safeRegex(value)) {
						new Notice("Unsafe regex! https://www.npmjs.com/package/safe-regex");
						return;
					}
		...
		new Setting(containerEl)
			...
			.addText((text) =>
				text.setValue(this.plugin.settings.mediaRootDirectory).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.viewImageEditor).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.viewImageInCPB).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.viewImageWithALink).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.viewImageOther).onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.pinMode).onChange(async (value) => {
		...
		this.switchSettingsDisabled(
			!this.plugin.settings.pinMode,
			pinMaximumSetting,
			pinCoverSetting
		);
		//endregion
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.imgTipToggle).onChange(async (value) => {
		...
		this.createPickrSetting(
			containerEl,
			"IMG_VIEW_BACKGROUND_COLOR_NAME",
			IMG_DEFAULT_BACKGROUND_COLOR
		);
		//endregion
		...
		new Setting(containerEl)
			...
			.addToggle((toggle) =>
				toggle.setValue(this.plugin.settings.imageBorderToggle).onChange(async (value) => {
		...
		let galleryNavbarDefaultColorSetting: Setting,
			galleryNavbarHoverColorSetting: Setting,
			galleryImgBorderToggleSetting: Setting,
			galleryImgBorderActiveColorSetting: Setting;
			...
			.addToggle((toggle) =>
				toggle
					...
					.onChange(async (value) => {
						...
						this.switchSettingsDisabled(
							!value,
							galleryNavbarDefaultColorSetting,
							galleryNavbarHoverColorSetting,
							galleryImgBorderToggleSetting,
							galleryImgBorderActiveColorSetting
						);
						await this.plugin.saveSettings();
					})
			);

		galleryNavbarDefaultColorSetting = this.createPickrSetting(
			containerEl,
			"GALLERY_NAVBAR_DEFAULT_COLOR_NAME",
			GALLERY_NAVBAR_DEFAULT_COLOR
		);
		galleryNavbarHoverColorSetting = this.createPickrSetting(
			containerEl,
			"GALLERY_NAVBAR_HOVER_COLOR_NAME",
			GALLERY_NAVBAR_HOVER_COLOR
		);
		...
		galleryImgBorderActiveColorSetting = this.createPickrSetting(
			containerEl,
			"GALLERY_IMG_BORDER_ACTIVE_COLOR_NAME",
			GALLERY_IMG_BORDER_ACTIVE_COLOR
		);

		this.switchSettingsDisabled(
			!this.plugin.settings.galleryNavbarToggle,
			galleryNavbarDefaultColorSetting,
			galleryNavbarHoverColorSetting,
			galleryImgBorderToggleSetting,
			galleryImgBorderActiveColorSetting
		...
		const moveTheImageSetting = new Setting(containerEl)
			...
			.then((setting) => {
				setting.controlEl.appendChild(
					createDiv("setting-editor-extra-setting-button hotkeys-settings-plus", (el) => {
						el.innerHTML = "+";
					})
				);
				setting.controlEl.appendChild(
					createDiv("setting-editor-extra-setting-button", (el) => {
	...
	checkDropdownOptions(code: string, setting: Setting): void {
		...
		const optionElList: HTMLCollectionOf<HTMLOptionElement> = setting.controlEl
			.getElementsByClassName("dropdown")[0]
			.getElementsByTagName("option");
		for (let i = 0, size = optionElList.length; i < size; i++) {
			if (code === MOVE_THE_IMAGE.CODE) {
				optionElList[i].disabled =
					optionElList[i].value === this.plugin.settings.moveTheImageHotkey;
			} else if (code === SWITCH_THE_IMAGE.CODE) {
				optionElList[i].disabled =
					optionElList[i].value === this.plugin.settings.switchTheImageHotkey;
			}
		}
	}
}
