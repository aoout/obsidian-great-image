...
export class MainContainerView extends ContainerView {
	...
	public initContainerViewDom = (containerEl: HTMLElement): ImgCto => {
		...
		if (!this.imgInfoCto.oitContainerViewEl) {
			...
			containerEl.appendChild(
				(this.imgInfoCto.oitContainerViewEl = createDiv("oit-main-container-view"))
			);
			// <div class="oit-main-container-view"> <div class="img-container"/> </div>
			this.imgInfoCto.oitContainerViewEl.append(
				(this.imgInfoCto.imgContainerEl = createDiv("img-container"))
			);
			...
			this.imgInfoCto.oitContainerViewEl.appendChild(
				(this.imgInfoCto.imgTipEl = createDiv())
			); // img-tip
			...
			this.imgInfoCto.oitContainerViewEl.appendChild(
				(this.imgInfoCto.imgFooterEl = createDiv())
			); // img-footer
			this.imgInfoCto.imgFooterEl.addClass("img-footer");

			// <div class="img-title"></div>
			this.imgInfoCto.imgFooterEl.appendChild(
				(this.imgInfoCto.imgTitleEl = createDiv("img-title"))
			); // img-title
			this.imgInfoCto.imgTitleEl.appendChild(
				(this.imgInfoCto.imgTitleNameEl = createSpan("img-title-name"))
			);
			this.imgInfoCto.imgTitleEl.appendChild(
				(this.imgInfoCto.imgTitleIndexEl = createSpan("img-title-index"))
			);
			...
			this.imgInfoCto.oitContainerViewEl.appendChild(
				(this.imgInfoCto.imgPlayerEl = createDiv("img-player"))
			); // img-player for full screen mode
			this.imgInfoCto.imgPlayerEl.appendChild(
				(this.imgInfoCto.imgPlayerImgViewEl = createEl("img"))
			);
	...
	public renderImgTitle = (name?: string, index?: string): void => {
		if (undefined !== name && null !== name) this.imgInfoCto.imgTitleNameEl?.setText(name);
	...
	protected switchImageOnGalleryNavBar = (event: KeyboardEvent, next: boolean) => {
		if (!this.checkHotkeySettings(event, this.plugin.settings.switchTheImageHotkey)) return;
