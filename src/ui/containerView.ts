...
export abstract class ContainerView {
	...
	protected constructor(
		...
		containerType: keyof typeof CONTAINER_TYPE,
		pinMaximum: number
	) {
		this.plugin = plugin;
	...
	protected checkStatus = (): boolean => {
		...
		if (this.imgInfoCto.oitContainerViewEl) {
			const containerElList: HTMLCollectionOf<Element> =
				document.getElementsByClassName(oitContainerViewClass);
	...
	protected restoreBorderForLastClickedImg = () => {
		...
		if (lastClickedImgStyle) {
			lastClickedImgStyle.setProperty(
				"border-width",
				this.lastClickedImgDefaultStyle.borderWidth
			);
			lastClickedImgStyle.setProperty(
				"border-style",
				this.lastClickedImgDefaultStyle.borderStyle
			);
			lastClickedImgStyle.setProperty(
				"border-color",
				this.lastClickedImgDefaultStyle.borderColor
			);
		}
	};
	//endregion
	...
	protected getMatchedImg = (): ImgCto => {
		...
		for (const img of this.imgInfoCto.imgList) {
			if (!earliestImg || earliestImg.mtime > img.mtime) earliestImg = img;
			if (img.popup) continue;
			return img;
		}
	...
	public refreshImg = (
		...
		if (imgSrc) {
			...
			realImg.src = imgSrc;
			imgCto.refreshImgInterval = setInterval(
				(realImg) => {
					if (realImg.width > 0 || realImg.height > 0) {
						...
						imgCto.imgViewEl.style.setProperty(
							"transform",
							imgCto.defaultImgStyle.transform
						);
						imgCto.imgViewEl.style.setProperty("filter", imgCto.defaultImgStyle.filter);
						imgCto.imgViewEl.style.setProperty(
							"mix-blend-mode",
							imgCto.defaultImgStyle.mixBlendMode
						);
					}
				},
				40,
	...
	public renderImgTip = (activeImg?: ImgCto) => {
		...
		if (
			activeImg &&
			this.imgInfoCto.imgTipEl &&
			activeImg.realWidth > 0 &&
			activeImg.curWidth > 0
		) {
			...
			if (this.plugin.settings.imgTipToggle) {
				...
				this.imgInfoCto.imgTipEl.style.setProperty(
					"font-size",
					isSingleDigit || 100 >= activeImg.curWidth ? "xx-small" : "x-small"
				);
	...
	public setImgViewDefaultBackground = (imgViewEl: HTMLImageElement) => {
		if (!imgViewEl) return;
		if (
			this.plugin.settings.imgViewBackgroundColor &&
			IMG_DEFAULT_BACKGROUND_COLOR != this.plugin.settings.imgViewBackgroundColor
		) {
			imgViewEl.removeClass("img-default-background");
			imgViewEl.style.setProperty(
				"background-color",
				this.plugin.settings.imgViewBackgroundColor
			);
		} else {
	...
	protected showPlayerImg = (activeImg: ImgCto) => {
		...
		this.imgInfoCto.imgPlayerEl.style.setProperty(
			"z-index",
			this.imgGlobalStatus.activeImgZIndex + 10 + ""
		);
	...
	protected addOrRemoveEvents = (matchedImg: ImgCto, isAdd: boolean) => {
		if (isAdd) {
			...
			if (!this.isPinMode()) {
				...
				this.imgInfoCto.oitContainerViewEl.addEventListener(
					"click",
					this.closeContainerView
				);
			}
			...
			matchedImg.imgViewEl.addEventListener("mousewheel", this.mousewheelViewContainer, {
				passive: true,
			});
		} else {
			if (!this.imgGlobalStatus.popup) {
			...
			if (!this.isPinMode()) {
				this.imgInfoCto.oitContainerViewEl.removeEventListener(
					"click",
					this.closeContainerView
				);
			}
	...
	private getAndUpdateActiveImg = (event: MouseEvent | KeyboardEvent): ImgCto => {
		...
		if (
			activeImg &&
			(!this.imgGlobalStatus.activeImg ||
				activeImg.index !== this.imgGlobalStatus.activeImg.index)
		) {
	...
	public clickImgToolbar = (
		event: MouseEvent,
		targetElClass?: string,
		activeImg?: ImgCto
	): void => {
