...
export class GalleryNavbarView {
	...
	public renderGalleryImg = async (imgFooterEl: HTMLElement) => {
		...
		const imgContextHash: string[] = this.getTargetImgContextHash(
			this.mainContainerView.getLastClickedImgEl(),
			activeView.containerEl,
			this.plugin.imgSelector
		);
		...
		if (0 <= targetImageIdx) {
			if (liElActive) {
				...
				if (this.settings.galleryImgBorderActive) {
					...
					liElActive.style.setProperty(
						"border-color",
						this.settings.galleryImgBorderActiveColor
					);
				}
			}

			this.galleryTranslateX =
				(document.documentElement.clientWidth || document.body.clientWidth) / 2.5 -
				targetImageIdx * 52;
	...
	private initGalleryNavbar = (imgFooterEl: HTMLElement) => {
		...
		if (!this.galleryNavbarEl) {
			...
			this.galleryNavbarEl.onmouseover = () => {
				this.galleryNavbarEl.style.setProperty(
					"background-color",
					this.settings.galleryNavbarHoverColor
				);
			};
			this.galleryNavbarEl.onmouseout = () => {
				this.galleryNavbarEl.style.setProperty(
					"background-color",
					this.settings.galleryNavbarDefaultColor
				);
			};
			// add events
	...
	private activateImage = (liEl: HTMLLIElement | HTMLElement, imgEL?: HTMLImageElement) => {
		...
		if (imgEL) {
			...
			this.mainContainerView.refreshImg(
				activeImg,
				imgEL.src,
				imgEL.alt || "",
				imgEL.dataset.index
	...
	private clickGalleryImg = (event: MouseEvent) => {
		...
		if (this.galleryListEl) {
			const liElList: HTMLCollectionOf<Element> =
				this.galleryListEl.getElementsByClassName("gallery-active");
	...
	public switchImage = (next: boolean) => {
		...
		const liElList: HTMLCollectionOf<HTMLLIElement> =
			this.galleryListEl.getElementsByTagName("li");
	...
	private getGalleryImgCache = (file: TFile): GalleryImgCacheCto => {
		...
		const galleryImgCache: GalleryImgCacheCto =
			GalleryNavbarView.GALLERY_IMG_CACHE.get(md5File);
